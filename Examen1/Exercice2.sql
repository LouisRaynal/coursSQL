---- Exercice 2

-- Question 1 : 0.75 pt
SELECT titre, genre, duree_secondes, taille_bytes, duree_secondes/taille_bytes
FROM Chansons
ORDER BY duree_secondes DESC
LIMIT 15
;

-- Question 2 : 0.75 pt
SELECT UPPER(titre), duree_secondes/60 AS duree_minutes
FROM Chansons
ORDER BY duree_minutes DESC
;

-- Question 3 : 0.5 pt
SELECT *
FROM Chansons
WHERE compositeur IS NOT NULL
;

-- Question 4 : 1 pt
SELECT titre, LENGTH(titre) AS "longueur du titre"
FROM Chansons
WHERE genre = 'Classical'
ORDER BY "longueur du titre" DESC
LIMIT 1
;

-- Question 5 : 0.5 pt
SELECT *
FROM Chansons
WHERE compositeur LIKE '%David Bowie%'
    OR compositeur LIKE '%Queen%'
;

-- Question 6 : 0.5 pt
SELECT titre, genre
FROM Chansons
WHERE genre IN ('Pop','Blues','Jazz')
;

-- Question 7 : 0.5 pt
SELECT DISTINCT format
FROM Chansons
;

-- Question 8 : 0.75 pt
SELECT format, COUNT(id_chanson), AVG(duree_secondes)
FROM Chansons
GROUP BY format
;

-- Question 9 : 0.75 pt
SELECT genre, COUNT(id_chanson)
FROM Chansons
WHERE compositeur IS NULL
GROUP BY genre
ORDER BY 2 ASC
;

-- Question 10 : 0.5 pt
SELECT titre, duree_secondes
FROM Chansons
WHERE duree_secondes BETWEEN 60 AND 120
    AND (titre LIKE 'me%' OR titre LIKE '%me') -- attention aux parenthèses
;

-- Question 11 : 1 pt
SELECT *
FROM Chansons
WHERE genre NOT IN ('Metal','Rock')
    AND duree_secondes >= 300
    AND titre NOT LIKE '% a%' -- et le titre ne contient pas ' a'
    AND id_album = 25
;

-- Question 12 : 0.5 pt

SELECT SUBSTR('Bonjour', 1, 3)
;

SELECT titre, artiste
FROM Albums
WHERE SUBSTR(titre,1,5) = SUBSTR(artiste,1,5)
;

-- Question 13 : 0.75 pt
SELECT  
    artiste AS Artiste,
    titre AS Album,
    artiste || ' - ' || titre AS "Artiste - Album"
FROM Albums
ORDER BY Artiste ASC
;

-- Question 14 : 0.75 pt
SELECT 
    al.titre AS "titre album",
    al.artiste AS Artiste,
    ch.titre AS "titre chanson"
FROM Albums AS al
INNER JOIN Chansons AS ch
    ON al.id_album = ch.id_album
;

-- Question 15 : 0.5 pt
SELECT
    al.titre, COUNT(ch.id_chanson) AS NbrChansons
FROM Albums AS al
INNER JOIN Chansons AS ch
    ON al.id_album = ch.id_album
GROUP BY al.titre
ORDER BY NbrChansons DESC
;