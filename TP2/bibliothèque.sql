...
-- numéro de téléphone (téléphone) - INTEGER
-- code postal (code_postal) - INTEGER
...
-- Voir ci-dessus le listing des informations complétées.
-- Remarque : pour la table abonnés, j'ai choisi d'utiliser le type INTEGER 
-- pour le numéro de téléphone et le code postal. Vous auriez pu utiliser le type VARCHAR(10)
-- et VARCHAR(5) respectivement si vous vouliez limiter la taille des saisies.
-- Ou bien INTEGER(10) et INTEGER(5) qui a le même effet (non vu dans le cours).
...
-- Veillez à ce que les clés primaires aient une contraite PRIMARY KEY,
-- et que les autres informations indispensables aient une contraite NOT NULL.
-- Pour la table livres, on considère une contrainte DEFAULT(0) sur le prix, pour définir
-- comme prix par défaut. On considère aussi la contrainte CHECK( prix >= 0 ) pour s'assuer
-- que le prix n'est pas négatif.

-- Il est aussi possible d'ajouter une contraite de table qui permet de s'assurer que la date
...
--    La phrase précédente se traduira par la contraite de table (lors de la création de la table emprunt) suivante :
